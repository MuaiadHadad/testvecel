...
@app.route('/administracoes', methods=['POST'])
def criar_administracao():
    dados_administracao = request.get_json()

    nova_administracao = Administracao(
        medicamento_id=dados_administracao['medicamento_id'],
        nomeMedicamento=dados_administracao['nomeMedicamento'],
        formaFarmaceutica=dados_administracao['formaFarmaceutica'],
        horaAdministracao=dados_administracao['horaAdministracao'],
        DataAdministracao=dados_administracao['date'],
        quantidade=dados_administracao['quantidade'],
        administrado=dados_administracao['administrado'],
        horaFoitomado=dados_administracao['horaFoitomado']
    )

    db.session.add(nova_administracao)
    db.session.commit()

    return jsonify({'message': 'Administração registrada com sucesso'}), 201
...
# Sua rota para adicionar administrações
...
# Sua rota para alterar o estado e a hora de administração
...
def alterar_estado_hora_administracao():
    try:
        ...
        administracao = Administracao.query.join(Medicamento).filter(Administracao.id==id_data,Medicamento.user_id == current_user).first()
...
# Sua rota para remover um medicamento e suas administrações associadas
# Rota para remover um medicamento e suas administrações associadas
...
def alterar_medicamento():
    try:
        ...
        if Medicamento.query.filter(Medicamento.nomeMedicamento == dados_medicamento['nomeMedicamento'],Medicamento.id_Medicamento != id_medicamento, Medicamento.user_id == current_user).first():
